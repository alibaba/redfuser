...
def _generate_online_expr(mod: IRModule, func_name: str) -> IRModule:
    # do schedule here
...
def GenerateOnlineExpr(mod: IRModule, ctx: tvm.transform.PassContext) -> IRModule:
    for gvar, func in mod.functions.items():
        if isinstance(func, PrimFunc):
            mod = _generate_online_expr(mod, gvar.name_hint)
    return mod
