...
class Module:
    ...
    def redfuser_ptpc_quant_gemm(A: T.Buffer((4096, 4096), "float16"), W_fp8: T.Buffer((4096, 4096), "float8_e4m3fn"), W_scale: T.Buffer((4096,), "float32"), T_o: T.Buffer((4096, 4096), "float16")):
        ...
        # with T.block("root"):
        input_0 = T.alloc_buffer((4096, 4096), "float16")
        T_max_elem = T.alloc_buffer((4096,), "float16")
        prev_T_max_elem = T.alloc_buffer((4096,), "float16")
        input_1 = T.alloc_buffer((4096, 4096), "float8_e4m3fn")
        ...
        for m in T.serial(4096, annotations={"bind": "vblockIdx.1", "name": "m"}):
            for k in T.serial(4096, annotations={"name": "k", "tag": "fused"}):
                with T.block("reduction0"):
                    ...
                    T.reads(A[v_m, v_k])
                    T.writes(input_0[v_m, v_k], prev_T_max_elem[v_m], T_max_elem[v_m])
                    with T.init():
                        T_max_elem[v_m] = T.float16(-65504.0)
                    prev_T_max_elem[v_m] = T_max_elem[v_m]
                    input_0[v_m, v_k] = T.fabs(A[v_m, v_k])
                    T_max_elem[v_m] = T.max(T_max_elem[v_m], input_0[v_m, v_k])
        for m in T.serial(4096, annotations={"bind": "vblockIdx.1", "name": "m"}):
            for n in T.serial(4096, annotations={"bind": "vblockIdx.0", "name": "n"}):
                for k in T.serial(4096, annotations={"name": "k", "tag": "fused"}):
                    with T.block("reduction1"):
                        v_m, v_n, v_k = T.axis.remap("SSR", [m, n, k])
                        T.reads(prev_T_max_elem[v_m], T_max_elem[v_m], A[v_m, v_k], W_fp8[v_n, v_k])
                        T.writes(rescale_factor_1[v_m], input_1[v_m, v_k], T_matmul_NT[v_m, v_n])
                        with T.init():
                            T_matmul_NT[v_m, v_n] = T.float32(0.0)
                        rescale_factor_1[v_m] = T.Cast("float32", prev_T_max_elem[v_m]) / T.Cast("float32", T_max_elem[v_m])
                        input_1[v_m, v_k] = T.Cast("float8_e4m3fn", T.Cast("float32", A[v_m, v_k]) * T.float32(448.0) / T.Cast("float32", T_max_elem[v_m]))
                        T_matmul_NT[v_m, v_n] = T_matmul_NT[v_m, v_n] * rescale_factor_1[v_m]
                        T_matmul_NT[v_m, v_n] = T_matmul_NT[v_m, v_n] + T.Cast("float32", input_1[v_m, v_k]) * T.Cast("float32", W_fp8[v_n, v_k])
        for m in T.serial(4096, annotations={"bind": "vblockIdx.1", "name": "m"}):
            for n in T.serial(4096, annotations={"bind": "vblockIdx.0", "name": "n"}):
                with T.block("epilogue0"):
                    ...
                    T_o[v_m, v_n] = T.Cast("float16", T_matmul_NT[v_m, v_n] * T.Cast("float32", T_max_elem[v_m]) / T.float32(448.0) * W_scale[v_n])
