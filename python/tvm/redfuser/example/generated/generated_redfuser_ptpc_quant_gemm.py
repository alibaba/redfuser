...
def redfuser_ptpc_quant_gemm():
    ...
    def kernel(
        ...
        W_fp8: T.Tensor([4096, 4096], "float8_e4m3fn"), 
        W_scale: T.Tensor([4096], "float32"), 
        o: T.Tensor([4096, 4096], "float16")
    ):
        with T.Kernel(32, 32) as (v_n_o, v_m_o):
            ...
            input_0 = T.alloc_fragment([128, 128], "float16")
            max_elem = T.alloc_fragment([128], "float16")
            prev_max_elem = T.alloc_fragment([128], "float16")
            input_1 = T.alloc_fragment([128, 128], "float8_e4m3fn")
            rescale_factor_1 = T.alloc_fragment([128], "float32")
            matmul_NT = T.alloc_fragment([128, 128], "float32")
            T.fill(max_elem[0:128], -65504.0)
            ...
            for v_k_o in T.Pipelined(0, 32, num_stages=1):
                ...
                for m_1_m, k_1_m in T.Parallel(128, 128):
                    input_0[m_1_m, k_1_m] = T.abs(A_1[m_1_m, k_1_m])
                T.reduce(input_0, max_elem, "max", 1, False)
                for m_1_m in T.Parallel(128):
                    rescale_factor_1[m_1_m] = T.Cast("float32", prev_max_elem[m_1_m]) / T.Cast("float32", max_elem[m_1_m])
                for m_1_m, k_1_m in T.Parallel(128, 128):
                    input_1[m_1_m, k_1_m] = T.Cast("float8_e4m3fn", T.Cast("float32", A_1[m_1_m, k_1_m]) * 448.0 / T.Cast("float32", max_elem[m_1_m]))
                for m_1_m, n_1_m in T.Parallel(128, 128):
                    matmul_NT[m_1_m, n_1_m] = matmul_NT[m_1_m, n_1_m] * rescale_factor_1[m_1_m]
                T.gemm(input_1, W_fp8_1, matmul_NT, transpose_B=True, policy=1)
            T.copy(W_scale[v_n_o * 128:v_n_o * 128 + 128], W_scale_1[0:128])
            for m_1, n_1 in T.Parallel(128, 128):
                o_1[m_1, n_1] = T.Cast("float16", matmul_NT[m_1, n_1] * T.Cast("float32", max_elem[m_1]) / 448.0 * W_scale_1[n_1])
