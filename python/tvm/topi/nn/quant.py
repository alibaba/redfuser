...
def per_axis_fp8_quant(x, axis = -1, reduce_axis_name="k", varargs_names=[]):
    ...
    def _compute_max(abs_elem, *indices):
        ...
        return tvm.te.max(abs_elem[eval_range], axis=k)
    ...
    abs_elem = te.compute(shape, lambda *indices: tvm.te.abs(x[indices]), name="T_abs_elem", varargs_names=allargs_names)
    ...
    scale_elem = te.compute(reduced_shape, lambda *indices: tvm.te.div(max_elem[indices], FP8_MAX).astype("float32"), name="T_scale_elem", varargs_names=varargs_names)
